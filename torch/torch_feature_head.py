...
for idx in range(0, len(train.filenames),batchsize):
    ...
    if idx + batchsize<len(train.filenames):
        ff = readtrain_img(train.filenames[idx:idx+batchsize])
        ff = [tr(x) for x in ff]
        ff = torch.stack(ff)
        ...
        ff = readtrain_img(train.filenames[idx:])
        ff = [tr(x) for x in ff]
        ff = torch.stack(ff)
        ff = model_conv(Variable(ff.cuda(1))).view(-1,featurenum)
        train_feature.append(ff.data.cpu().numpy())
...
for idx in range(0, len(test.filenames),batchsize):
    ...
    if idx + batchsize<len(test.filenames):
        ...
        ff = readtest_img(test.filenames[idx:idx+batchsize])
        ff = [tr(x) for x in ff]
        ff = torch.stack(ff)
        ff = model_conv(Variable(ff.cuda(1))).view(-1,featurenum)
        ...
        ff = readtest_img(test.filenames[idx:])
        ff = [tr(x) for x in ff]
        ff = torch.stack(ff)
        ff = model_conv(Variable(ff.cuda(1))).view(-1,featurenum)
        test_feature.append(ff.data.cpu().numpy())
...
with h5py.File('./data/train_head_hun/224_'+network, "w") as f:
    f.create_dataset("train", data=train_feature)

with h5py.File('./data/test_head/224_'+network, "w") as f:
